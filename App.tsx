...
const App: React.FC<{ firebaseUser: User | null }> = ({ firebaseUser }) => {
  ...
  const updateSupplier = useCallback((oldName: string, newName: string) => {
    ...
    setCategories((prev) =>
      prev.map((c) =>
        c.supplier === oldName ? { ...c, supplier: newName } : c
      )
    );
  }, []);
  ...
  const updateCategory = useCallback(
    async (oldName: string, supplier: string, newName: string) => {
      if (!oldName || !newName) return;

      // Find the Firestore ID based on the name/supplier
      const categoryToUpdate = (categories as any[]).find(
        (c) => c.name === oldName && c.supplier === supplier
      );

      if (categoryToUpdate && categoryToUpdate.id) {
        try {
          await updateDocument("categories", categoryToUpdate.id, {
            name: newName,
          });
        } catch (error) {
  ...
  const removeCategory = useCallback(
    async (name: string, supplier: string) => {
      if (!name || !supplier) return;

      // Find the Firestore ID
      const categoryToDelete = (categories as any[]).find(
        (c) => c.name === name && c.supplier === supplier
      );
  ...
  const showToast = useCallback(
    (message: string, type: "success" | "error" | "warning" = "success") => {
      ...
      setTimeout(
        () =>
          setToast((prevToast) => (prevToast?.id === id ? null : prevToast)),
        4000
      );
    },
    []
    ...
    return new Promise<any>((resolve) => {
      try {
        const unsub = subscribeToCollection("customers", (items: any[]) => {
          const match =
            (items || []).find(
              (c) => (c.referralCode || "").trim() === referralCode.trim()
            ) || null;
          try {
            unsub();
          } catch (e) {
  ...
  const addLead = useCallback(
    async (leadData: Omit<Lead, "id" | "createdAt">, referralCode?: string) => {
      try {
        ...
        if (referralCode && referralCode.trim()) {
          ...
          let referrer = customers.find(
            (c) => c.referralCode === referralCode.trim()
          );
  ...
  async (newOrder: {
    ...
    try {
      ...
      const insufficient = itemDetails.find(
        (d) => d.product!.quantity < d.requested.quantity
      );
      if (insufficient) {
        console.log("[addOrder] insufficient stock", {
          productId: insufficient!.product!.id,
          have: insufficient!.product!.quantity,
          want: insufficient!.requested.quantity,
        });
      ...
      const datePrefix = new Date()
        ...
        .slice(0, 10)
        .replace(/-/g, "");
      const dailyOrderCount = orders.filter((o) =>
        o.invoiceNumber?.startsWith(`INV-${datePrefix}`)
      ).length;
      ...
      // ✅ STEP 1: Write order document to Firestore FIRST
      let createdId: string;
      try {
        createdId = await addToCollection("orders", orderPayload);
        console.log("[addOrder] created order id ->", createdId);
      } catch (err) {
        console.error("[addOrder] CRITICAL: Failed to create order:", err);
        showToast("Failed to create order. Transaction rolled back.", "error");
        return { success: false, message: "Order creation failed", error: err };
      }

      // ✅ STEP 2: Update all products and stock history
      const updateErrors: { productId: string; error: any }[] = [];
      ...
      for (const it of itemDetails) {
        ...
        try {
          ...
          await updateDocument("products", p.id, rest);
          console.log("[addOrder] updated product", p.id, "newQty=", newQty);
        } catch (err) {
          console.error(
            "[addOrder] ERROR updating product quantity:",
            p.id,
            err
          );
          updateErrors.push({ productId: p.id, error: err });
          // Continue with other items but track this error
          continue;
        }

        // Write stock history (best-effort, but important)
        try {
          await addStockHistoryEntry({
            productId: p.id,
            productName: p.name,
          ...
          console.warn(
            "[addOrder] WARNING: failed to write stock history for",
            p.id,
            err
          );
          // Don't fail the order, but log it
          // This is a "nice-to-have" — the order and stock are more important
        }
      }

      // ✅ STEP 3: Check if any critical updates failed
      if (updateErrors.length > 0) {
        console.error(
          "[addOrder] PARTIAL FAILURE: Some products failed to update:",
          updateErrors
        );
        showToast(
          `⚠️ Order created but ${updateErrors.length} product(s) failed to update stock. Manual review needed!`,
          "warning"
        );
        // Still return success because order WAS created
        return {
          success: true,
          message: `Invoice #${invoiceNumber} created (with stock update warnings)`,
          order: { id: createdId, ...orderPayload },
        };
      }
        ...
        if (status === "Paid") {
          ...
          if (customer) {
            ...
            const customerPaidOrders = orders.filter(
              (o) => o.customerId === customer.id && o.paymentStatus === "Paid"
            );
  ...
  // Firestore-backed updateInvoiceStatus (invoice-specific logic)
  const updateInvoiceStatus = useCallback(
    async (
      ...
      console.log("[updateInvoiceStatus] called ->", {
        orderId,
        status,
        paymentMethod,
      });
      try {
        // Find order in local cache
        const order = orders.find((o) => o.id === orderId);
        if (!order) {
        ...
        // If it was paid, handle referral logic
        if (status === "Paid") {
          const customer = customers.find((c) => c.id === order.customerId);
          if (customer) {
            // Check if this is the customer's first paid order
            const customerPaidOrders = orders.filter(
              (o) => o.customerId === customer.id && o.paymentStatus === "Paid"
            );
            if (customerPaidOrders.length === 0 && !customer.referralCode) {
        ...
        const msg = err?.message || "Failed to update invoice status";
        showToast(msg, "error");
      }
    },
    [orders, customers, showToast]
  ...
  const markRewardAsPaid = useCallback(
    async (referralId: string) => {
      try {
        // update local state optimistically
        setReferrals((prev) =>
          prev.map((r) =>
            r.id === referralId ? { ...r, status: "RewardPaid" } : r
          )
        );
        // persist
  ...
  const overdueFollowUps = useMemo(() => {
    ...
    return leads.filter(
      (lead) => lead.followUpDate && new Date(lead.followUpDate) < today
    );
  }, [leads]);
  ...
  const ToastComponent = () => {
    ...
    return (
      <div
        className={`fixed bottom-5 right-5 bg-white dark:bg-slate-800 shadow-lg rounded-lg p-4 border-l-4 ${
          colors[toast.type]
        } flex items-center gap-3 z-[100] animate-fade-in-up`}
      >
        {" "}
        {icons[toast.type]}{" "}
  ...
  return (
    <CrmContext.Provider value={contextValue}>
      <div
        className={`flex h-screen bg-slate-100 text-slate-900 dark:bg-slate-900 dark:text-slate-100 font-sans`}
      >
                      ...
                      overdueFollowUps.map((lead) => (
                        <button
                          ...
                          onClick={() => {
                            ...
                            setViewingItem({ type: "lead", id: lead.id });
                            setIsNotificationsOpen(false);
                          }}
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    showToast &&
                      showToast("Sign out failed. Check console.", "error");
                  }
                }}
