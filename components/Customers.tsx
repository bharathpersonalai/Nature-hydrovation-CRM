...
type CustomerModalView = 'details' | 'newOrder' | 'viewInvoice' | 'viewReceipt' | 'viewReferralSlip';
...
const Customers: React.FC = () => {
    ...
    const [customerModalView, setCustomerModalView] = useState<CustomerModalView>('details');
    ...
    const filteredCustomers = useMemo(() => {
        ...
        if (purchaseHistoryFilter !== 'all' || locationFilter) {
            tempCustomers = tempCustomers.filter(customer => {
                ...
                if (purchaseHistoryFilter === 'high-value') purchaseMatch = totalSpent > 10000;
                else if (purchaseHistoryFilter === 'repeat') purchaseMatch = orderCount > 1;
    ...
    const SortableHeader: React.FC<{ children: React.ReactNode, sortKey: SortableKey }> = ({ children, sortKey }) => {
        const isSorting = sortConfig?.key === sortKey;
        const icon = isSorting 
            ? sortConfig.direction === 'ascending' ? <ChevronUpIcon className="w-4 h-4 ml-1.5" /> : <ChevronDownIcon className="w-4 h-4 ml-1.5" />
            : <ChevronsUpDownIcon className="w-4 h-4 ml-1.5 opacity-0 group-hover:opacity-40" />;

        return (
            <th scope="col" className="px-6 py-3">
                <button onClick={() => requestSort(sortKey)} className="group flex items-center uppercase font-medium">
                    {children} {icon}
                </button>
            </th>
        );
    ...
    const hasProductsInStock = useMemo(() => products.some(p => p.quantity > 0), [products]);
    
    const customerReferralStats = useMemo(() => {
        if (!selectedCustomer) return { count: 0, earnings: 0, referrals: [] };
        const customerReferrals = referrals.filter(r => r.referrerId === selectedCustomer.id);
        ...
        return {
            ...
            referrals: customerReferrals.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
        };
    ...
    const handleCustomerFormSubmit = (e: React.FormEvent) => {
        ...
        if (editingCustomer) {
            ...
            const { referralCode, ...customerData } = customerFormData as typeof emptyCustomer;
    ...
    const handleDocumentAction = (action: 'print' | 'pdf', order: Order, type: 'invoice' | 'receipt') => {
            ...
            canvas.toBlob(async (blob) => {
                if (blob) {
                    ...
                    if (navigator.canShare && navigator.canShare({ files: [file] })) {
                        ...
                        showToast("Sharing files is not supported on this browser.", "warning");
                    }
                } else {
    ...
    return (
        <div className="p-6 md:p-8">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-200">Customers</h1>
                <div className="flex items-center gap-2">
                    <button onClick={handleExportCustomers} className="flex items-center gap-2 bg-slate-600 text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-slate-700 transition-colors">
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-4 p-4 bg-slate-50 dark:bg-slate-800 rounded-lg border dark:border-slate-700 items-center">
                <h3 className="text-md font-semibold text-slate-700 dark:text-slate-200 flex-shrink-0">Segments:</h3>
                <div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-4">
                 ...
                 <button onClick={handleClearFilters} className="ml-4 flex-shrink-0 text-sm flex items-center gap-1.5 text-slate-500 hover:text-brand-primary dark:text-slate-400 dark:hover:text-brand-primary transition-colors">
                    ...
                    {customers.length > 0 ? (
                        sortedCustomers.length > 0 ? (
                            <table className="w-full text-sm text-left text-slate-500 dark:text-slate-400">
                                ...
                                <tbody>
                                    {sortedCustomers.map(customer => (
                                        <tr key={customer.id} className="bg-white border-b hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-700 dark:hover:bg-slate-700">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-4">
                                                    <button onClick={() => handleViewCustomer(customer)} className="font-medium text-brand-primary hover:underline">
                                                        View
                                                    </button>
                                                    <button onClick={() => handleOpenEditModal(customer)} className="text-slate-400 hover:text-brand-primary transition-colors" aria-label="Edit customer">
                            ...
                            <div className="text-center py-16">
                                <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200">No Customers Found</h3>
                                <p className="text-slate-500 mt-2 dark:text-slate-400">Your search and filter combination did not return any results.</p>
                            </div>
                        )
                    ) : (
                         <div className="text-center py-16">
                            <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200">No customers yet</h3>
                            <p className="text-slate-500 mt-2 dark:text-slate-400">Convert qualified leads or add a customer manually.</p>
                        </div>
                    )}
                </div>
            </div>

            <Modal isOpen={isAddEditModalOpen} onClose={() => { setIsAddEditModalOpen(false); setEditingCustomer(null); }} title={editingCustomer ? "Edit Customer" : "Add New Customer"}>
                <form onSubmit={handleCustomerFormSubmit} className="space-y-4">
                    <input type="text" name="name" placeholder="Name" value={customerFormData.name} onChange={(e) => setCustomerFormData(p => ({...p, name: e.target.value}))} className="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-primary focus:border-brand-primary dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200" required />
                    ...
                    {!editingCustomer && (
                        <>
                            ...
                            <input type="text" name="referralCode" placeholder="Referral Code (Optional)" value={(customerFormData as any).referralCode || ''} onChange={(e) => setCustomerFormData(p => ({...p, referralCode: e.target.value}))} className="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-primary focus:border-brand-primary dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200" />
                        </>
                    )}
                    <div className="flex justify-end pt-4">
                        <button type="submit" className="bg-brand-primary text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-brand-dark transition-colors">{editingCustomer ? "Save Changes" : "Add Customer"}</button>
                    </div>
                </form>
            </Modal>
            ...
            {selectedCustomer && (
                <Modal 
                    ...
                    title={
                        customerModalView === 'details' ? selectedCustomer.name :
                        customerModalView === 'newOrder' ? `New Order for ${selectedCustomer.name}` :
                        customerModalView === 'viewInvoice' ? `Invoice ${viewingOrder?.invoiceNumber}` :
                      ...
                      (customerModalView === 'viewInvoice' && viewingOrder) ? (
                        <>
                            ...
                            {viewingOrder.paymentStatus === 'Unpaid' ? (
                              ...
                              <button onClick={() => setCustomerModalView('viewReceipt')} className="flex items-center gap-2 bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-blue-700 transition-colors">
                                <ReceiptIcon className="w-5 h-5" />
                                View Receipt
                              </button>
                            )}
                            <button onClick={() => handleDocumentAction('pdf', viewingOrder, 'invoice')} className="flex items-center gap-2 bg-slate-600 text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-slate-700 transition-colors">
                                <PdfIcon className="w-5 h-5" /> Download PDF
                            </button>
                            <button onClick={() => handleDocumentAction('print', viewingOrder, 'invoice')} className="flex items-center gap-2 bg-brand-primary text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-brand-dark transition-colors">
                        ...
                        <>
                           <button type="button" onClick={() => setCustomerModalView('details')} className="bg-slate-200 text-slate-800 font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-slate-300 transition-colors dark:bg-slate-600 dark:text-slate-200 dark:hover:bg-slate-500">
                            ...
                            <button onClick={() => setCustomerModalView('viewInvoice')} className="flex items-center gap-2 bg-slate-600 text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-slate-700 transition-colors">
                                <FileTextIcon className="w-5 h-5" /> View Invoice
                            </button>
                            <button onClick={() => handleDocumentAction('pdf', viewingOrder, 'receipt')} className="flex items-center gap-2 bg-slate-600 text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-slate-700 transition-colors">
                                <PdfIcon className="w-5 h-5" /> Download PDF
                            </button>
                            <button onClick={() => handleDocumentAction('print', viewingOrder, 'receipt')} className="flex items-center gap-2 bg-brand-primary text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-brand-dark transition-colors">
                                <PrinterIcon className="w-5 h-5" /> Print Receipt
                            </button>
                        </>
                    ) : (customerModalView === 'viewReferralSlip' && viewingReferralSlip) ? (
                         <>
                            <button type="button" onClick={() => setCustomerModalView('details')} className="bg-slate-200 text-slate-800 font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-slate-300 transition-colors dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600">
                                Back to Details
                            </button>
                            <button onClick={handleShareSlip} className="flex items-center gap-2 bg-slate-200 text-slate-800 font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-slate-300 transition-colors dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600">
                                <ShareIcon className="w-5 h-5" /> Share
                            </button>
                            <button onClick={handleDownloadSlip} className="flex items-center gap-2 bg-brand-primary text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-brand-dark transition-colors">
                ...
                >
                    {customerModalView === 'details' && (
                        <div className="space-y-4">
                            <div>
                                <h4 className="font-semibold text-slate-800 dark:text-slate-200">Contact Info</h4>
                            ...
                            <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <button 
                                        ...
                                        title={!hasProductsInStock ? "No products in stock to create an order" : "Create a new order"}
                                ...
                                {activeCustomerTab === 'orders' && (
                                    getCustomerOrdersGrouped(selectedCustomer.id).length > 0 ? (
                                        ...
                                        <div className="text-center py-4 bg-slate-50 rounded-lg dark:bg-slate-800/50">
                                            <p className="text-sm text-slate-500 dark:text-slate-400">No orders yet for this customer.</p>
                                        </div>
                                    )
                                )}
                                {activeCustomerTab === 'payments' && (
                                     getCustomerPaymentHistory(selectedCustomer.id).length > 0 ? (
                                        <div className="overflow-x-auto max-h-60 border rounded-lg dark:border-slate-700">
                                            <table className="w-full text-sm text-left">
                                                <thead className="text-xs text-slate-600 dark:text-slate-400 uppercase sticky top-0 bg-slate-50 dark:bg-slate-700/80 backdrop-blur-sm">
                                                    <tr>
                                        ...
                                        <div className="text-center py-4 bg-slate-50 rounded-lg dark:bg-slate-800/50">
                                            <p className="text-sm text-slate-500 dark:text-slate-400">No payment history found.</p>
                                        </div>
                                     )
                                )}
                                {activeCustomerTab === 'referrals' && (
                                    <div className="space-y-4">
                                        <div className="p-4 bg-slate-50 rounded-lg dark:bg-slate-800/50">
                                            {selectedCustomer.referralCode ? (
                                                <div className="text-center">
                                                    <h4 className="text-sm font-semibold text-slate-500 dark:text-slate-400">YOUR REFERRAL CODE</h4>
                                                    <div className="my-2 p-2 bg-brand-light dark:bg-brand-dark/30 border-2 border-dashed border-brand-primary rounded-lg">
                                                        <code className="text-2xl font-bold text-brand-primary">{selectedCustomer.referralCode}</code>
                                                    </div>
                                                    <button onClick={() => {
                                                        navigator.clipboard.writeText(selectedCustomer.referralCode || '');
                                                        showToast("Referral code copied to clipboard!");
                                                    }} className="text-sm font-medium text-brand-primary hover:underline">Copy Code</button>
                                                    <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
                                                        <div className="bg-white dark:bg-slate-700 p-3 rounded-lg shadow-sm">
                                                            <div className="font-semibold text-slate-500 dark:text-slate-400">Successful Referrals</div>
                                                            <div className="text-2xl font-bold text-slate-800 dark:text-slate-200">{customerReferralStats.count}</div>
                                                        </div>
                                                        <div className="bg-white dark:bg-slate-700 p-3 rounded-lg shadow-sm">
                                                            <div className="font-semibold text-slate-500 dark:text-slate-400">Total Rewards Earned</div>
                                                            <div className="text-2xl font-bold text-slate-800 dark:text-slate-200">₹{customerReferralStats.earnings.toLocaleString()}</div>
                                                        </div>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div className="text-center py-4">
                                                    ...
                                                    <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">This customer will receive a referral code after their first paid order.</p>
                                                </div>
                                            )}
                                        </div>

                                        <div className="p-4 bg-slate-50 rounded-lg dark:bg-slate-800/50">
                                            <h4 className="text-sm font-semibold text-slate-500 dark:text-slate-400">CUSTOMIZE REFERRAL SLIP</h4>
                                            <div className="mt-2 flex items-center gap-4">
                                                <div className="w-24 h-14 bg-slate-200 dark:bg-slate-700 rounded-md flex items-center justify-center">
                                                    {referralSlipBackground ? (
                                                        <img src={referralSlipBackground} alt="Referral slip background" className="w-full h-full object-cover rounded-md" />
                                                    ) : (
                                                        <ImageIcon className="w-6 h-6 text-slate-400" />
                                                    )}
                                                </div>
                                                <div>
                                                    <input type="file" ref={backgroundUploadRef} onChange={handleBackgroundUpload} accept="image/*" className="hidden" />
                                                    <button onClick={() => backgroundUploadRef.current?.click()} className="flex items-center gap-2 text-sm font-medium bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 px-3 py-1.5 rounded-md hover:bg-slate-50 dark:hover:bg-slate-600">
                                        ...
                                        <div>
                                            <h4 className="text-sm font-semibold text-slate-500 dark:text-slate-400">REFERRAL HISTORY</h4>
                                            {customerReferralStats.referrals.length > 0 ? (
                                                            ...
                                                            {customerReferralStats.referrals.map(referral => (
                                                                <tr key={referral.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                                                    ...
                                                                    <td className="px-4 py-2 text-center">
                                                                        <button onClick={() => { setViewingReferralSlip(referral); setCustomerModalView('viewReferralSlip'); }} className="flex items-center justify-center mx-auto gap-1.5 font-medium text-brand-primary hover:underline">
                                                ...
                                                <div className="text-center py-4 mt-2 bg-slate-50 rounded-lg dark:bg-slate-800/50">
                                                    <p className="text-sm text-slate-500 dark:text-slate-400">No referral history yet.</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                    ...
                    {customerModalView === 'newOrder' && (
                        <form onSubmit={handleAddOrderSubmit} className="space-y-4">
                            ...
                            <div className="flex justify-end pt-4 gap-2">
                                ...
                                <button type="submit" className="bg-brand-primary text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-brand-dark transition-colors">Place Order</button>
                            </div>
                        </form>
                    )}
                    {customerModalView === 'viewInvoice' && viewingOrder && (
            ...
            <Modal isOpen={isPaymentModalOpen} onClose={() => setIsPaymentModalOpen(false)} title="Record Payment">
                <div className="space-y-4">
                    <div>
                        <label htmlFor="paymentMethod" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Payment Method</label>
                        <select 
                            id="paymentMethod" 
                    ...
                    <div className="flex justify-end pt-2 gap-2">
                        <button type="button" onClick={() => setIsPaymentModalOpen(false)} className="bg-slate-200 text-slate-800 font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-slate-300 transition-colors dark:bg-slate-600 dark:text-slate-200 dark:hover:bg-slate-500">
                            Cancel
                        </button>
                        <button onClick={handleConfirmPayment} className="bg-green-600 text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-green-700 transition-colors">
                            Confirm Payment
                        </button>
                    </div>
                </div>
